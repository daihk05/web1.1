		...
		function getPlayer($source, $playerOptions = array(), $actualFile = "") {
			...
			if (is_feed()) {
				...
				switch ( $this->options["rssAlternate"] ) {
					case "download":
						...
						for ($i = 0; $i < count($files); $i++) {
							...
							$links .= '<a href="' . $files[$i] . '">' . __('Download audio file', 'cosmotheme') . ' (' . $fileName . ')</a><br />';
						}
						return $links;
						break;
				...
				if (strlen($this->options["flashAlternate"]) > 0) {
					...
					$playerCode = '<p class="audioplayer_container"><span style="display:block;padding:5px;border:1px solid #dddddd;background:#f8f8f8" id="' . $playerElementID . '">' . sprintf(__('Audio clip: Adobe Flash Player (version 9 or above) is required to play this audio clip. Download the latest version <a href="%s" title="Download Adobe Flash Player">here</a>. You also need to have JavaScript enabled in your browser.', 'cosmotheme'), 'http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash&amp;promoid=BIOW') . '</span></p>';
				}
