...
if (!class_exists('simple_modal_login')) {
	class simple_modal_login {
		...
		function __construct() {
			...
			if (!is_admin()) {
				...
				add_action('wp_footer', array($this, 'login_footer'),105);
	...
	<form name="loginform" id="loginform" action="%s" method="post">
		...
		<p class="nav">',
            ...
            if(has_action( 'social_connect_form' )){ /*this action is defined if sociall connect plugin is installed*/
				do_action( 'social_connect_form' );
			}else if( !( options::get_value( 'social' , 'facebook_app_id' ) == '' || options::get_value( 'social' , 'facebook_secret' ) == '' ) ){
